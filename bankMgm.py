...
# function to handle valid credentials
def is_number(s):
    try:
        float(s)
        return 1
...
def write(master, name, oc, pin):
    ...
    text = "{:>25}".format('Date and Time') + \
            "{:>15}".format('Credit') + \
            "{:>15}".format('Debit') + \
            "{:>20}".format('Balance') + '\n'
    frec.write(text)
    text1 = "{:>25}".format(str(strftime("[%Y-%m-%d] [%H:%M:%S]  ", gmtime()))) + \
            "{:>15}".format(oc) + \
            "{:>15}".format('') + \
            "{:>20}".format(oc) + '\n'
    frec.write(text1)
...
def disp_tr_hist(accnt):
    ...
    l1 = tk.Message(disp_wn, text="Your Transaction History:", font=("Times", 16),
                padx=100, pady=20, width=1000, bg='blue4', fg='SteelBlue1', relief='raised')
    l1.pack(side='top')
...
def logged_in_menu(accnt, name):
    ...
    l_title = tk.Message(rootwn, text='BANK MANAGEMENT SYSTEM', relief='raised', width=2000, padx=600, pady=0,
                            fg='white', bg='blue4', justify='center')
    l_title.config(font=('Arial', '50', 'bold'))
    l_title.pack(side='top')
    label = tk.Label(text='Logged in as: '+name, relief='raised', bg='blue3', font=('Times', 16),
                                fg='white', anchor='center', justify='center')
    label.pack(side='top')
    img2 = tk.PhotoImage(file='assets/credit.gif')
    myimg2 = img2.subsample(2, 2)
    img3 = tk.PhotoImage(file='assets/debit.gif')
    myimg3 = img3.subsample(2, 2)
    img4 = tk.PhotoImage(file='assets/balance.gif')
    myimg4 = img4.subsample(2, 2)
    img5 = tk.PhotoImage(file='assets/transaction.gif')
    ...
    img6 = tk.PhotoImage(file='assets/logout.gif')
...
def validate(name, acc_num, pin):
    f1 = open(str(acc_num)+'.txt', 'r')
    fpin = f1.readline().strip()
    f1.readline()
    facc = f1.readline().strip()
    fname = f1.readline().strip()
    f1.close()
    # print(fpin, fname, facc)
    if name==fname and acc_num==facc and pin==fpin:
        return True
    else:
        return False
...
def log_in(master):
    ...
    b.pack(side='top')
    b1 = tk.Button(loginwn, text='HOME', font=("Times", 16), relief='raised', bg='blue', fg='white', command=lambda: home_return(loginwn))
    b1.pack(side='top')
    loginwn.bind("<Return>", lambda x:check_log_in(loginwn, e1.get().strip(), e2.get().strip(), e3.get().strip()))

# function to create new account window
def Create():
    ...
    l_title = tk.Message(crwn, text='BANK MANAGEMENT SYSTEM', relief='raised', width=2000, padx=600, pady=0,
                            fg='white', bg='blue4', justify='center')
    l_title.config(font=('Arial', '50', 'bold'))
    l_title.pack(side='top')
...
# function to display main menu window
def Main_Menu():
    rootwn = tk.Tk()
    rootwn.geometry('1600x500')
    ...
    l_title.pack(side='top')
    imgc1 = tk.PhotoImage(file='assets/new.gif')
    imglo = tk.PhotoImage(file='assets/login.gif')
    ...
    img6 = tk.PhotoImage(file='assets/quit.gif')
    myimg6 = img6.subsample(2, 2)
